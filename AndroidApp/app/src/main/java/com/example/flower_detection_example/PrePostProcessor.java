...
public class PrePostProcessor {
    ...
    // model output is of size 25200*27
    private static int mOutputRow = 25200; // as decided by the YOLOv5 model for input image of size 640*640
    private static int mOutputColumn = 32; // left, top, right, bottom, score and 27 class probability
    ...
    static ArrayList<Result> nonMaxSuppression(ArrayList<Result> boxes, int limit, float threshold) {
        ...
        Collections.sort(boxes,
                new Comparator<Result>() {
                    @Override
                    public int compare(com.example.flower_detection_example.Result o1, com.example.flower_detection_example.Result o2) {
        ...
        for (int i=0; i<boxes.size() && !done; i++) {
            if (active[i]) {
                com.example.flower_detection_example.Result boxA = boxes.get(i);
                selected.add(boxA);
                if (selected.size() >= limit) break;

                for (int j=i+1; j<boxes.size(); j++) {
                    if (active[j]) {
                        com.example.flower_detection_example.Result boxB = boxes.get(j);
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                com.example.flower_detection_example.Result result = new com.example.flower_detection_example.Result(cls, outputs[i*32+4], rect);
                results.add(result);
            }
        }
